package com.ujuit.datamove.createClass.gencode.modelcontent;
import java.io.File;
import java.io.IOException;
import java.util.List;

import com.ujuit.datamove.createClass.gencode.reflect.FieldColumnPojo;
import com.ujuit.datamove.createClass.gencode.utils.ClassUtil;
import com.ujuit.datamove.createClass.gencode.utils.GenFileUtil;

public class ModelEntityUtils {

	/**
	 * @param packageName 包名（org.qisys.db.sec.equinfo）
	 * @param modelName 表名（equinfo）
	 * @param tableDesc 表描述（沪深股票键盘精灵）
	 * @param fieldList 表属性列表
	 * @return 文件名称（没有要后缀名）
	 */
	...
	public String createContext(List<FieldColumnPojo> fieldList){
		...
		if(fieldList != null && fieldList.size() > 0){
			for (int i = 0; i < fieldList.size(); i++) {
				...
				sb.append("	 */"+ClassUtil.NEXT_ROW);
				sb.append("	private "+ClassUtil.fieldType(fcp.getValidteType()) + " "+fcp.getFieldName()+";"+ClassUtil.NEXT_ROW);
			...
			for (int i = 0; i < fieldList.size(); i++) {
				...
				sb.append("	 *"+fcp.getLabelName()+"\r\n");
				sb.append("	 */"+ClassUtil.NEXT_ROW);
				sb.append("	public "+ClassUtil.fieldType(fcp.getValidteType()) + " "+ClassUtil.GET+methodName+"() {"+ClassUtil.NEXT_ROW);
				...
				sb.append("	 *"+fcp.getLabelName()+"\r\n");
				sb.append("	 */"+ClassUtil.NEXT_ROW);
				sb.append("	public void "+ClassUtil.SET+methodName+"("
					+ClassUtil.fieldType(fcp.getValidteType())+" "+fcp.getFieldName()+") "
				    + "{"+ClassUtil.NEXT_ROW);
